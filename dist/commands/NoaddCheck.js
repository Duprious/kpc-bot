...
exports.NoaddCheckCommand = void 0;
const discord_js_1 = require("discord.js");
const NoaddCheckCommand = async (msg, args, client) => {
    ...
    if (!adminCommandChatsIDs.includes(msg.channel.id))
        return;
    let player;
    try {
    ...
    const chatChannelTier1 = client.channels.cache.get('735126005349482558');
    const chatChannelTier2 = client.channels.cache.get('785143121662705664');
    const filter = (m) => m.author.id === msg.author.id && !Number.isNaN(parseInt(m.content));
                            ...
                            msg.channel.send('Objective amount of TeamMate 3:   ').then(msg => {
                                msg.channel.awaitMessages({ filter: filter, max: 1, time: 25000, errors: ["time"] })
                                    ...
                                    msg.channel.send('Tier 1 or Tier 2? (1 or 2)').then(msg => {
                                        msg.channel.awaitMessages({ max: 1, time: 15000, errors: ["time"] }).then(messages => {
                                            ...
                                            msg.channel.send('Calculating....').then(msg => {
                                                msg.channel.awaitMessages({ filter: filter, time: 10 })
                                                    ...
                                                    const lessthantenp = new discord_js_1.EmbedBuilder()
                                                        .setTitle('Done!')
                                                        .setDescription('User is less than 10% and has been NoAdded!')
                                                        .setColor('Red')
                                                        .setTimestamp();
                                                    const lessthanfifteenp = new discord_js_1.EmbedBuilder()
                                                        .setTitle('Done!')
                                                        .setDescription('User is less than 15% and has been NoAdded!')
                                                        .setColor('Red')
                                                        .setTimestamp();
                                                    const lessthanfifteenpbutwin = new discord_js_1.EmbedBuilder()
                                                        .setTitle('Done!')
                                                        .setDescription('User is less than 15% but won the game so not NoAdded!')
                                                        .setColor('Green')
                                                        .setTimestamp();
                                                    const usergood = new discord_js_1.EmbedBuilder()
                                                        .setTitle('Done!')
                                                        .setDescription('User is not less than 15% and is good to go!')
                                                        .setColor('Green')
                                                        .setTimestamp();
                                                    const Calculationsembed = new discord_js_1.EmbedBuilder()
                                                        .setTitle('Calculations Done!')
                                                        .addFields({ name: 'Total Objective:', value: `${total}`, inline: true }, { name: '10% Objective:', value: `${tenp}`, inline: true }, { name: '15% Objective:', value: `${fifteenp}`, inline: true })
                                                        .setColor('Green')
                                                        .setTimestamp();
                                                    let chatChannel;
                                                    if (whichtier == 1) {
...
exports.NoaddCheckCommand = NoaddCheckCommand;
